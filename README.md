...
```
yarn add @teamerror/Eaileys
```

Use the edge version (no guarantee of stability, but latest fixes + features)
```
yarn add github:teamerror/Eaileys
```

Then import your code using:
``` ts 
import makeWASocket from '@teamerror/Eaileys'
```
...
``` ts
import makeWASocket, { DisconnectReason } from '@teamerror/Eaileys'
...
``` ts
import makeWASocket, { BufferJSON, useMultiFileAuthState } from '@teamerror/Eaileys'
...
``` ts
import makeWASocket, { makeInMemoryStore } from '@teamerror/Eaileys'
...
``` ts
import { MessageType, MessageOptions, Mimetype } from '@teamerror/Eaileys'
...
- **MiscGenerationOptions**: some extra info about the message. It can have the following __optional__ values:
    ...
    const info: MessageOptions = {
        ...
        mimetype: Mimetype.pdf, /* (for media messages) specify the type of media (optional for all media types except documents),
                                    import {Mimetype} from '@teamerror/Eaileys'
                                */
...
import { downloadMediaMessage } from '@teamerror/Eaileys'
